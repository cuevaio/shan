...
import type { StrokePoint } from "./motion/stroke";
...
export function createMotionRouteHandlers(options: MotionRouteOptions = {}) {
  const apiKey = () => options.apiKey ?? process.env.NEBIUS_API_KEY;
  const modelId = () => options.modelId ?? (process.env.NEBIUS_MODEL?.trim() || DEFAULT_NEBIUS_MODEL);
  const request = options.fetch ?? globalThis.fetch;

  async function complete(model: string, points: StrokePoint[], jsonMode: boolean) {
    return request(`${options.baseUrl ?? NEBIUS_BASE_URL}chat/completions`, {
      method: "POST",
      cache: "no-store",
  ...
  async function POST(incoming: Request) {
    ...
    try {
      ...
      return Response.json({ status: "error", message: "The stroke needs coordinates." }, { status: 400 });
    }
    ...
    if (!apiKey()) {
      return Response.json({ status: "waiting", message: "The model step is waiting on NEBIUS_API_KEY." });
    }
