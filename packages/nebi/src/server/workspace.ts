...
async function writeAtomically(path: string, content: string) {
  ...
  const temporary = `${path}.nebi-${crypto.randomUUID()}.tmp`;
  try {
...
export function applyChangesEffect(root: string, changes: DraftChange[]) {
  return Effect.gen(function*() {
    ...
    yield* Effect.forEach(changes, (change) =>
      ...
      Effect.catchAll((applyError) =>
        Effect.forEach([...applied].reverse(), (change) => restoreChange(workspace.root, change)).pipe(
          Effect.catchAll((rollbackError) => Effect.fail(new WorkspaceError({
