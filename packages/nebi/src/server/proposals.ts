...
const STORE_KEY = Symbol.for("nebi.proposal-store");
const ACTIVE_KEY = Symbol.for("nebi.active-previews");
...
function storagePath(root: string) {
  ...
  return join(tmpdir(), "nebi-agent", `${projectId}.json`);
}

async function persist(proposal: StoredProposal) {
  const path = storagePath(proposal.root);
  await mkdir(join(tmpdir(), "nebi-agent"), { recursive: true, mode: 0o700 });
...
export function keepProposalEffect(id: string, expectedRoot: string) {
