...
async function project() {
  const root = await mkdtemp(join(tmpdir(), "nebi-test-"));
  roots.push(root);
...
describe("WorkspaceDraft", () => {
  ...
  test("exposes typed failures to Effect callers", async () => {
    ...
    const result = await Effect.runPromise(Effect.either(applyChangesEffect(root, draft.getChanges())));

    expect(Either.isLeft(result)).toBe(true);
    if (Either.isLeft(result)) {
      expect(result.left._tag).toBe("WorkspaceError");
      expect(result.left.message).toContain("changed after");
    }
  });
